...
file = open("/var/www/html/180.txt","w")
frest = open("/var/www/html/180sure.txt","r+")
...
try:
	...
	data, addr = sock.recvfrom(1024) # buffer size is 1024 bytes
        ...
        data = data.split("\\",1)[0]
        data = int(data)
        res = "30180|"+datetime.now().strftime('%H:%M:%S')+"|"+str(data)
        print "Res: "+str(res)
        file.write(str(res))
	...
	try:
		...
		fi = open("/var/www/html/frestart180.txt","w")
