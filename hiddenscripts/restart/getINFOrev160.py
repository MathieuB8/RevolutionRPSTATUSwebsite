...
file = open("/var/www/html/160.txt","w")
frest = open("/var/www/html/160sure.txt","r+")
...
try:
	...
	data, addr = sock.recvfrom(1024) # buffer size is 1024 bytes
        ...
        data = data.split("\\",1)[0]
        data = int(data)
        res = "30160|"+datetime.now().strftime('%H:%M:%S')+"|"+str(data)
        print "Res: "+str(res)
        file.write(str(res))
	...
	try:
		...
		fi = open("/var/www/html/frestart160.txt","w")
